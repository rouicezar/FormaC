...
- 当前没有可用飞书测试应用凭据，因此只能确认签名回调、去重和回复客户端契约，尚未把“真实租户收发成功”标记为完成。
...
- 当前本机仍未发现可用飞书测试应用凭据或 Feishu/Lark 环境变量，因此真实租户私聊、群聊 `@机器人`、重复事件和内外身份收发验收仍未完成。

## 下一步

取得可用的飞书测试应用凭据后，在开放平台配置事件订阅并完成真实租户私聊、群聊 `@机器人`、重复事件和内外身份范围验收；随后进入 `/app/history` 与个人记录统计，把统一记录按当前登录/绑定身份映射到用户端。
