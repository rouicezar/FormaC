...
export function KnowledgePage() {
  ...
  async function scanNow() {
    ...
    try {
      ...
      let keepGoing = true;
      let batch = 0;
      let finalResult: ScanReport | null = null;
      while (keepGoing) {
        batch += 1;
        let result = await startManualScan(prefix ? { prefix } : { limit: DEFAULT_SCAN_BATCH_SIZE, changedOnly: true });
        setScan(result);
        if (!TERMINAL_SCAN_STATUSES.has(result.status)) {
          setMessage(prefix ? `后台扫描已开始：${prefix}` : `后台分批扫描已开始：第 ${batch} 批，每批最多 ${DEFAULT_SCAN_BATCH_SIZE} 个新增或变化文件。`);
        ...
        finalResult = result;
        if (!TERMINAL_SCAN_STATUSES.has(result.status)) {
          setMessage("后台扫描仍在运行。可以稍后回到本页查看最新状态；再次点击会继续扫描剩余文件。");
          return;
        }
        if (result.status === "failed" || result.failed) break;
        keepGoing = !prefix && result.changed_only && result.total === DEFAULT_SCAN_BATCH_SIZE;
        if (keepGoing) setMessage(`第 ${batch} 批已完成，正在继续扫描剩余新增或变化文件。`);
      }
      setMessage(finalResult?.failed ? "扫描已完成，但存在处理失败的文件。" : "分批增量扫描完成，索引已同步。");
    }
  ...
  return <main className="page-content admin-config-page">
    ...
    {busy === "loading" ? <div className="config-loading"><Loader2 className="spin" />正在读取配置</div> : <div className="admin-config-grid">
      ...
      <section className="config-card full"><div className="config-card-title"><Database /><div><h2>手动扫描</h2><p>可先扫描一个相对路径或目录；留空时自动分批扫描新增或变化文件。</p></div></div><label>扫描范围（可选）<input value={scanPrefix} onChange={(event) => setScanPrefix(event.target.value)} placeholder="例如：public/12_长期关注/Anthropic news.md 或 public/09_视频口播稿/" /></label><p className="field-hint">当前知识库文件较多。留空扫描时系统会按批次继续处理，避免一次超长扫描中断后必须重来。</p>{!scan ? <div className="inline-empty">尚未从本页发起扫描。</div> : <><div className={`inline-status ${scan.status === "failed" || scan.failed ? "error" : scan.status === "running" ? "" : "success"}`} role="status">{scan.status === "running" ? `后台扫描中，已处理 ${scan.processed}/${scan.total || "?"} 个文件。` : `扫描状态：${scan.status}`}{scan.changed_only ? <small>模式：仅新增或变化文件</small> : null}{scan.prefix ? <small>扫描范围：{scan.prefix}</small> : null}{scan.current_path ? <small>当前文件：{scan.current_path}</small> : null}</div><div className="scan-metrics">{[["新增", scan.added], ["更新", scan.updated], ["删除", scan.deleted], ["跳过", scan.skipped], ["失败", scan.failed]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></>}{scan?.errors.length ? <div className="scan-errors">{scan.errors.map((item, index) => <p key={index}><strong>{item.path}</strong>{item.error}</p>)}</div> : null}</section>
    </div>}
  </main>;
}
