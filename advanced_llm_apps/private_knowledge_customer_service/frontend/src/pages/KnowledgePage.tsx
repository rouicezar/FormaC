...
export function KnowledgePage() {
  ...
  async function scanNow() {
    ...
    try {
      ...
      let result = await startManualScan(prefix ? { prefix } : {});
      setScan(result);
      if (!TERMINAL_SCAN_STATUSES.has(result.status)) {
        setMessage(prefix ? `后台扫描已开始：${prefix}` : "后台全量扫描已开始。当前知识库较大，本页会自动刷新扫描状态。");
  ...
  return <main className="page-content admin-config-page">
    ...
    {busy === "loading" ? <div className="config-loading"><Loader2 className="spin" />正在读取配置</div> : <div className="admin-config-grid">
      ...
      <section className="config-card full"><div className="config-card-title"><Database /><div><h2>手动扫描</h2><p>可先扫描一个相对路径或目录；留空时才执行全量扫描。</p></div></div><label>扫描范围（可选）<input value={scanPrefix} onChange={(event) => setScanPrefix(event.target.value)} placeholder="例如：public/12_长期关注/Anthropic news.md 或 public/09_视频口播稿/" /></label><p className="field-hint">当前知识库文件较多，全量扫描会持续较久。建议先填具体文件或目录确认链路正常。</p>{!scan ? <div className="inline-empty">尚未从本页发起扫描。</div> : <><div className={`inline-status ${scan.status === "failed" || scan.failed ? "error" : scan.status === "running" ? "" : "success"}`} role="status">{scan.status === "running" ? `后台扫描中，已处理 ${scan.processed}/${scan.total || "?"} 个文件。` : `扫描状态：${scan.status}`}{scan.prefix ? <small>扫描范围：{scan.prefix}</small> : null}{scan.current_path ? <small>当前文件：{scan.current_path}</small> : null}</div><div className="scan-metrics">{[["新增", scan.added], ["更新", scan.updated], ["删除", scan.deleted], ["跳过", scan.skipped], ["失败", scan.failed]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></>}{scan?.errors.length ? <div className="scan-errors">{scan.errors.map((item, index) => <p key={index}><strong>{item.path}</strong>{item.error}</p>)}</div> : null}</section>
    </div>}
  </main>;
}
