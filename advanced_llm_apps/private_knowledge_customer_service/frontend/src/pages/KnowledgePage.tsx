...
export function KnowledgePage() {
  ...
  return <main className="page-content admin-config-page">
    ...
    {busy === "loading" ? <div className="config-loading"><Loader2 className="spin" />正在读取配置</div> : <div className="admin-config-grid">
      ...
      <section className="config-card full"><div className="config-card-title"><Database /><div><h2>最近手动扫描</h2><p>本页只展示当前会话触发的真实扫描结果。</p></div></div>{!scan ? <div className="inline-empty">尚未从本页发起扫描。</div> : <><div className={`inline-status ${scan.status === "failed" || scan.failed ? "error" : scan.status === "running" ? "" : "success"}`} role="status">{scan.status === "running" ? `后台扫描中，已处理 ${scan.processed}/${scan.total || "?"} 个文件。` : `扫描状态：${scan.status}`}{scan.current_path ? <small>当前文件：{scan.current_path}</small> : null}</div><div className="scan-metrics">{[["新增", scan.added], ["更新", scan.updated], ["删除", scan.deleted], ["跳过", scan.skipped], ["失败", scan.failed]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></>}{scan?.errors.length ? <div className="scan-errors">{scan.errors.map((item, index) => <p key={index}><strong>{item.path}</strong>{item.error}</p>)}</div> : null}</section>
    </div>}
  </main>;
}
